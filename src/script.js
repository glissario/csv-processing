...
function stringToArray(string) {
  ...
  const articleArray = string.split('jpg');
  const firstItem = articleArray[0].split('Bildname');
  const newArticle = [];
...
function delLine() {
  ...
  for (let i = 0; i < lines.length; i++) {
    const lineIndex = lines[i].getAttribute('line-index');
    if (index === lineIndex) {
...
function getToChangeData() {
  ...
  const currentValueLabel = document.querySelector('#current-value');
  currentValueLabel.removeChild(currentValueLabel.firstChild);
  const currentValueNode = document.createTextNode(event.target.textContent);
  currentValueLabel.appendChild(currentValueNode);

  const inputfield = document.querySelector('#change-input-text');
  inputfield.focus();
...
function saveChange() {
  ...
  const node = document.createTextNode(saveText.value);
...
function activeRow() {
  ...
  for (let i = 0; i < active.length; i++) {
    if (active[i].checked) {
      const activeIndex = active[i].getAttribute('line-index');
      return activeIndex;
    }
  }
}
...
myForm.addEventListener('submit', function (e) {
  ...
  reader.onload = function (e) {
    ...
    /* eslint-disable */ const articles = stringToArray(text);
    createLines(globalArticleArray);
  };
